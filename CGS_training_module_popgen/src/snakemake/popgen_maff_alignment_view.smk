...
rule cat_all_consensus:
	input:
		consensus_fa="results/consensus_seq/{smp}_consensus.fa"
	output:
		mafft_catconsensus="results/maff_alignment/{smp}_mafft_catconsensus.out"
	message: """ --- concatenate all consensus fasta """
	shell: """
		time cat {input.consensus_fa} > {output.mafft_catconsensus}
	"""
	
rule maff_align:
	input:
		concatfasta="results/maff_alignment/{smp}_mafft_catconsensus.out"
	output:
		maff_out="results/maff_alignment/{smp}_mafft_align.out"
	message: """ --- mafft alignment """
	shell: """
		time mafft {input.concatfasta} > {output.maff_out}
	"""